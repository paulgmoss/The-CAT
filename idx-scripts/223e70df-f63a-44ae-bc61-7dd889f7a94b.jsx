            ...
            <React.Fragment>
                ...
                <div className="what-summary">
                  ...
                  <div className="what-summary-insights">
                    <PrincipleAccordion title="A CLO's weighting is a claim about how much of the assessment measures it." triggerStyle={{color:'var(--c-red)',backgroundColor:'var(--c-red-soft)'}} tagColor="var(--c-red)">
                    ...
                    <PrincipleAccordion title="The mapping and the teaching sequence are the same decision." triggerStyle={{color:'var(--c-red-2)',backgroundColor:'var(--c-red-2-soft)'}} tagColor="var(--c-red-2)">
...
function EssenceStage() {
  ...
  const DURATION = 18, HOLD = 5, CYCLE = DURATION + HOLD;
  ...
  React.useEffect(() => {
    ...
    function step(ts) {
      if (visibleRef.current) {
        ...
        const t = ((ts - start) / 1000) % CYCLE;
        setProgress(t < DURATION ? t / DURATION : 1);
      } else {
        start = null;
      }
    ...
    rafRef.current = requestAnimationFrame(step);
    const io = new IntersectionObserver(([entry]) => { visibleRef.current = entry.isIntersecting; }, { threshold: 0.2 });
  ...
  return (
    <div className="essence-stage" ref={ref}>
      <HeroDemo progress={progress} playing={visibleRef.current} />
    </div>);
}
